...
g <- ggplot(win, aes(x = year, y = total_wins)) +
  geom_boxplot()
  labs(title = "Total Wins by Year, Player, and Surface",
       x = "Year",
       y = "Total Wins") 
...
```{r}
#6 calculating technical indicators in serve
serve <- atp %>%
filter(!is.na(w_svpt)& 
       !is.na(w_1stIn) & 
       !is.na(w_1stWon) &  
...
```{r}
#7 calculating average technical indicators in serve group by winners 
...
```
```{r}
#8 visualizing the relationship between matches performance and serve technical indicators
...
```
```{r}
#10 caculating the difference between winner and loser in serving technical indicators
...
```{r}
#12 specifying Top3 atp players in serving technical indicators in the last 3 years
